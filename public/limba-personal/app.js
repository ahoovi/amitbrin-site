...
(() => {
  ...
  function startQuestion(){
    ...
    <div class="session-dock"><span class="dock-symbol">${icon('mic')}</span><div><strong id="session-status">${state.mode==='diagnostic'?'מיפוי בכתב':'תרגול עצמי בקול'}</strong><small>GPT-Live עדיין לא מחובר</small></div><button class="secondary" id="pause-session" aria-pressed="false">${icon('pause')} רגע, אני קורא/ת</button><button class="text-button" id="back-to-question">בחזרה לשאלה</button></div></main>${footer()}`;
  ...
  async function init() {
    ...
    try {
      const [data,diagnostic] = await Promise.all([api('me'),api('diagnostic')]); state.user = data.user; state.events = data.events; state.storage=data.storage; state.diagnosticItems=diagnostic.items;state.diagnosticVersion=diagnostic.version;
