...
const greeting=messages[0].event_id;
v.receive({type:'session.instructions.appended',client_event_id:'unrelated'});assert.equal(messages.length,1);
v.receive({type:'session.instructions.appended',client_event_id:greeting});assert.equal(messages.at(-1).type,'session.commentary.append','greeting prompt follows matching acknowledgment');
