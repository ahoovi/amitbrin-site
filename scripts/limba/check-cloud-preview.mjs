...
if(!share)throw new Error('Set LIMBA_PREVIEW_SHARE_URL to the approved preview sharing URL');
const base=new URL(share).origin;
if(!base.endsWith('-ahoovis-projects.vercel.app'))throw new Error('Unexpected test destination');
const gate=await fetch(share,{redirect:'manual'});
const accessCookie=gate.headers.getSetCookie().map(s=>s.split(';')[0]).join('; ');
...
try{
 ...
 const before=await (await call('me',undefined,amit)).json();assert.equal(before.storage,'cloud');
 ...
 console.log('PASS: external preview, both passwords, secure sessions, cloud write, independent login readback and account isolation.');
}finally{
